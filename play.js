 ...
 */
async function ensureActiveDevice(preferredDeviceId) {
...
const playMusic = {
    ...
    schema: {
        uri: z
            .string()
            .optional()
            .describe('Spotify URI to play (e.g. spotify:track:xxx, spotify:album:xxx)'),
        context_uri: z
            .string()
            .optional()
        ...
        try {
            ...
            await handleSpotifyRequest(async (spotifyApi) => {
                ...
                if (resolvedType === 'track') {
                    ...
                    await spotifyApi.player.startResumePlayback(activeDeviceId, spotifyUri, undefined, offset !== undefined ? { position: offset } : undefined);
                }
            });
            return {
...
const addTracksToPlaylist = {
    ...
    description: 'Add tracks or podcast episodes to a Spotify playlist. ' +
        'Accepts Spotify track IDs, episode IDs, or full Spotify URIs (e.g. spotify:episode:xxx).',
    schema: {
        playlistId: z.string().describe('The Spotify ID of the playlist'),
        trackIds: z
            .array(z.string())
            .describe('Array of Spotify IDs or URIs to add. ' +
        ...
        try {
            ...
            return {
                content: [
                    {
                        type: 'text',
                        text: `Successfully added ${trackIds.length} item${trackIds.length === 1 ? '' : 's'} to playlist (ID: ${playlistId})`,
                    },
                ],
            };
        }
            ...
            return {
                content: [
                    {
                        type: 'text',
                        text: `Error adding items to playlist: ${error instanceof Error ? error.message : String(error)}`,
                    },
                ],
            };
        }
