...
export async function createSpotifyApi() {
    ...
    if (config.accessToken && config.refreshToken) {
        const now = Date.now();
        const shouldRefresh = !config.expiresAt || config.expiresAt <= now + 5 * 60 * 1000;
        if (shouldRefresh) {
            console.error('Access token expired or missing expiration time, refreshing...');
            try {
                const tokens = await refreshAccessToken(config);
                config.accessToken = tokens.access_token;
...
async function refreshAccessToken(config) {
    ...
    if (!response.ok) {
        ...
        if (errorCode === 'invalid_grant') {
            ...
            saveSpotifyConfig(config);
            throw new Error('Spotify refresh token is no longer valid (invalid_grant) and has been discarded. Please run "npm run auth" to re-authenticate.');
        }
...
export async function authorizeSpotify() {
    ...
    const authPromise = new Promise((resolve, reject) => {
        ...
        server.listen(Number.parseInt(port), '127.0.0.1', () => {
            ...
            console.log('');
            open(authorizationUrl).catch(async (_error) => {
