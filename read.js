...
function formatShow(show, i) {
    return `${i + 1}. "${show.name}" by ${show.publisher} (${show.total_episodes} episodes) - ID: ${show.id}`;
}
const searchSpotify = {
    name: 'searchSpotify',
    description: 'Search for tracks, albums, artists, playlists, podcast episodes, or shows on Spotify. ' +
    ...
    schema: {
        query: z
            .string()
            .describe('The search query. Matches title, description, and publisher for podcasts.'),
        type: z
            .enum(SEARCH_TYPES)
            .describe('The type of item to search for: track, album, artist, playlist, episode (podcast episode), or show (podcast series)'),
        limit: z
            .number()
            .min(1)
            .max(10)
        ...
        offset: z
            ...
            .min(0)
            .optional()
            .describe('Index of the first result to return (default: 0). Combine with limit to page past the first 10 results.'),
    },
        ...
        try {
            ...
            if (type === 'episode') {
                ...
                if (!ids) {
                    ...
                    const full = await spotifyFetch('episodes', {
                ...
                const results = await handleSpotifyRequest(async (spotifyApi) => {
                    return await spotifyApi.search(query, [type], undefined, limitValue, offsetValue);
                });
                if (type === 'track' && results.tracks) {
                    formattedResults = results.tracks.items
                        ...
                        const popularity = typeof track.popularity === 'number'
    ...
    handler: async (args, _extra) => {
        ...
        const formattedTracks = playlistTracks.items
            ...
            if (track.type === 'episode') {
                return formatEpisode(track, offset + i);
            }
...
const getTopTracks = {
    name: 'getTopTracks',
    description: "Get the current user's top (most-played) tracks over a given time range. " +
        'This is the closest thing Spotify exposes to listening statistics.',
    schema: {
        timeRange: z
            .enum(TIME_RANGES)
            .optional()
            .describe('Time range: short_term (~4 weeks), medium_term (~6 months), or long_term (~1 year). Default: medium_term.'),
        limit: z
            .number()
            .min(1)
            .max(50)
    ...
    handler: async (args, _extra) => {
        ...
        const top = await handleSpotifyRequest(async (spotifyApi) => {
            return await spotifyApi.currentUser.topItems('tracks', timeRange, limit);
        });
...
const getTopArtists = {
    name: 'getTopArtists',
    description: "Get the current user's top (most-played) artists over a given time range. " +
        'This is the closest thing Spotify exposes to listening statistics.',
    schema: {
        timeRange: z
            .enum(TIME_RANGES)
    ...
    handler: async (args, _extra) => {
        const { timeRange = 'medium_term', limit = 20 } = args;
        const top = await handleSpotifyRequest(async (spotifyApi) => {
            return await spotifyApi.currentUser.topItems('artists', timeRange, limit);
        });
        if (top.items.length === 0) {
            return {
                content: [
